...
import { MAX_Y, MIN_Y } from 'src/pages/SplashPage/view'
  ...
  useEffect(() => {
    const canUserMoveBottomSheet = () => {
      ...
      if (touchMove.movingDirection === 'down') {
        ...
        return (content.current?.scrollTop as number) <= 0
      }
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      if (canUserMoveBottomSheet()) {
        ...
        // sheet.current?.style.setProperty('transform', `translateY(60px)`)
      } else {
    ...
    const handleTouchEnd = (e: TouchEvent) => {
      ...
      // Snap Animation
      const currentSheetY = sheet.current?.getBoundingClientRect().y as number;
